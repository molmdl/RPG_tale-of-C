...
Plans:
- [ ] 05.3-01-PLAN.md — WT-aligned structure load convention spec (the SC1 design artifact: when it fires [branch-node on_enter after the reverse-mutation edit, NOT the restore safety-net], conditionality [approach x conformational_difference, OQ-DM2 honored], substructure cmd.super align [align_sele per-enzyme Phase 7 field], presentation [overlay/replace, side-by-side deferred to Phase 6], honest text_teaching + hero-identity anti-confusion, the op="align" MolAction shape [target=mobile, args["reference"]=fixed] + the molops dispatch SPEC deferred to Phase 6, downstream contracts for 5.4/6/7, the 8 open questions) (Wave 1)
