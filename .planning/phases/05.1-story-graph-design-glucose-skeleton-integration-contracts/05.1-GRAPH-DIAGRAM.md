...
A visual summary of the 55-node glucose skeleton (`data/story_glucose/`) for human
...
**Post-expansion state (Plans 05.1-12/13 + 05.2-01):** 55 nodes (was 43); 21 endings (was
11; 1T+3G+2N+15B — the Bad tier grew 5 → 15: 5 → 14 via 05.1-12, then 14 → 15 via
05.2-01); start node `intro.preface` (was
`intro.select`); +9 bad-endings nodes (Req 1: 6 1a unknown-pool + 3 1b
known-consequence) + 2 preface nodes (Req 2: `intro.preface` Beat A +
`intro.shell_glucose` Beat B) + 1 bad-ending node (Phase 5.2: `bad.denature_ph_change`,
pH-denaturation, 1a `edit:unknown`). The 14 edit-allowed count + the 0 single-Continue
...
graph TD
    ...
    subgraph gly["glycolysis.json"]
        ...
        GPR["gly.pfk_restored<br/>↩️ RESTORATION BRANCH (Phase 7 plan 12)<br/>edit resi 209 chain A→GLY (framing) + load 4PFK wt + align"]
    ...
    subgraph tca["tca.json"]
        ...
        TAR["tca.aconitase_restored<br/>↩️ RESTORATION BRANCH (Phase 7 plan 12)<br/>edit resi 85 chain A→SER + load 1ACO wt + align"]
    ...
    subgraph bad["bad_endings.json"]
        ...
        BDP["bad.denature_ph_change *(NEW)*<br/>BAD: 1a unknown pool (pH-denaturation)"]
    end
    ...
    %% Restoration branch (Phase 7 plan 12): router-only entry (EditRouter
    %% routes known edits straight to the branch node -- NO incoming
    %% choice.goto edge, hence drawn as a note, not a spatial edge).
    GPR -->|"Continue / Observe"| GF
    GPK -->|"Continue"| GY
    ...
    TC --> TA --> TS
    %% Restoration branch (Phase 7 plan 12): router-only entry.
    TAR -->|"Continue / Observe"| TS
    ...
    class ACR,BL,BR,BC,BK,BEC,BST,BBA,BLIC,BPL,BMA,BAS,BSC,BCL,BDP badEnd
...
## Ending-Tier Distribution (21 endings — was 11; the Bad tier grew 5 → 15 via Plans 05.1-12/13 + 05.2-01)
...
| Bad | 15 | `anaer.crisis`, `bad.lost_connection`, `bad.released_from_host`, `bad.cycle_trap_host_death`, `bad.critical_residue_break` (existing 5) + `bad.enzyme_collapse`, `bad.wrong_substrate_trapped`, `bad.broken_active_site`, `bad.lost_in_cytosol`, `bad.proton_leak`, `bad.misfolded_aggregate` (1a, 6) + `bad.active_site_destroyed`, `bad.substrate_channel_blocked`, `bad.cofactor_lost` (1b, 3) + `bad.denature_ph_change` (1a, 1 — Phase 5.2 extensibility) |
...
known-consequence), then 14 → 15 via Plan 05.2-01 (+1: `bad.denature_ph_change`,
1a unknown-pool pH-denaturation); True/Good/Normal UNCHANGED.
...
structural, so the BFS sees them reachable; the `edit.prompt` stub now has 13
structural choices: 3 existing + 9 new from 05.1-12 + 1 new from 05.2-01).
...
### Phase 7 Restoration Topology (Plan 07-12)

Plan 07-12 implements the 07-01 DC-A outcome — the ONLY sanctioned topology
change of Phase 7: **55 → 57 nodes, 21 endings UNCHANGED** (both additions are
non-ending). The two fixed restoration branch nodes (`gly.pfk_restored` in
glycolysis.json + `tca.aconitase_restored` in tca.json) are the branch_node
targets the EditRouter routes known (correct reverse-mutation) edits to once
plan 14 binds `edits.json`. Both are NON-ending, NOT edit-allowed,
`claim_ids: []` (text/claims are plans 08/13), and re-enter the main path via
2 choices (Continue + Observe → `gly.fbp_to_pyruvate` / `tca.shuffle`).
Entry is ROUTER-ONLY (no incoming `choice.goto` edge —
`engine.apply_player_edit` routes directly to `branch_node`), so the BFS
neither reaches nor needs them; in the regenerated `05.1-graph.txt` /
`05.1-graph.svg` (rerun via `python3.6 tools/render_story_graph.py`,
"Summary: 57 nodes") they appear in the unreached/structural section. Each
`on_enter` = the 05.3 §2 (iii) restoration-reveal sequence
`[edit → load <enzyme>_wt → align (method=super, align_sele="name CA") →
show_as cartoon]`:

- `gly.pfk_restored`: edit `resi 209 and chain A` → `GLY` (the RECORDED
  game-design framing per 07-02 batch A decision 2 — 4PFK resi 209 is
  actually HIS; no bacterial residue mapping is asserted; the source node's
  pre-edit applyEdit is OMITTED) + `load pdb:4PFK {"object":"pfk_wt"}`.
- `tca.aconitase_restored`: edit `resi 85 and chain A` → `SER` — the DERIVED
  human↔bovine mapping (`tools/aconitase_mapping_probe.py`, SMOKE_RESULT:
  PASS): human ACO2 **S112** (UniProt Q99798) ↔ bovine **1ACO chain A resi 85**
  (SER) via a real Needleman-Wunsch alignment (96.4% identity) of the human
  sequence vs the cmd.iterate-extracted 1ACO sequence, cross-checked by
  1ACO's own DBREF record (PDB = UniProt P20004 − 27; 112 − 27 = 85) and the
  catalytic Ser642 anchor (1ACO 642 = SER). + `load pdb:1ACO
  {"object":"aconitase_wt"}` per D6. Chain case is EMPIRICAL: PyMOL matches
  `chain A` (uppercase) only.

### Phase 5.2 Extensibility Convention (Plan 05.2-01)

Phase 5.2 is the **FIRST intentional mutation of the frozen 5.1 skeleton**
(54→55 nodes, 20→21 endings, Bad 14→15). It adds +1 bad-ending node
(`bad.denature_ph_change`, pH-denaturation, 1a `edit:unknown`) to
`bad_endings.json` + +1 structural choice (path L) to the `edit.prompt` stub
(12→13 choices). The extensibility convention is locked by a parametrized
`subTest` (`test_all_edit_prompt_bad_endings_reachable_convention`) that
dynamically iterates `edit.prompt.choices` and auto-verifies future Phase 7
additions WITHOUT test edits. The runtime `edits.json` `bad_ending_pool` is
UNCHANGED (2 ids — runtime-pool expansion is Phase 7 content). ZERO code
change; the new frozen-at-55 state is established by `05.2-VERIFICATION`.
...
- **Bad-ending extensibility convention (Plan 05.2-01)**: structural bad-ending addition = +1 node in `bad_endings.json` + +1 structural choice in `edit.prompt` stub (`edit:unknown` continues the A-L global path sequence) + bump count assertions + the parametrized `subTest` auto-verifies; runtime `edits.json` `bad_ending_pool` expansion is Phase 7. See [Phase 5.2 Extensibility Convention](#phase-52-extensibility-convention-plan-052-01).
