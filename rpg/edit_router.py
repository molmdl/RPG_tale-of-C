...
``rpg.story`` data model + validator -- NO ``pymol``, NO ``random``).
...
- :class:`EditIntent` (defined in :mod:`rpg.story.model`) is the routing INPUT
...
the apply_edit helper's job (``rpg/pymol_layer/``), triggered by the routed
...
- NO pymol/PyQt5 imports (the Phase 1 AST gate scans ``rpg/`` root). NO
...
from rpg.story.model import EditIntent  # noqa: F401 (re-exported for callers)
from rpg.story.validate import Issue, _is_ending
...
def validate_edits_table(edits_table, story_nodes):
    ...
    Returns a list of :class:`rpg.story.validate.Issue` (empty = valid). Checks:
    ...
    ``_is_ending`` helper from :mod:`rpg.story.validate`.
    """
    issues = []
