...
of ``rpg/`` and stays in the domain tier (ARCHITECTURE.md Pattern 1).
...
import ``rpg.story.graph`` -- this keeps Plan 03 (validate) independent of
...
def collect_claim_ids(story_path):
    ...
    ``rpg.story.graph`` -- keeping Plan 03 (validate) independent of Plan 02
