# rpg/pymol_layer/molops.py -- Phase 3 Plan 03-03 MolOps + Phase 4 Plan 04-05
...
#
# Translates a MolAction (the pure-data carrier from rpg/story/model.py) into
...
#
# GATE EXCLUSION: lives in rpg/pymol_layer/ which tools/check_imports.py
# excludes via SKIP_DIRS = {"pymol_layer","ui","__pycache__"} -- the domain
# tier (rpg/ root) stays pure-Python (no pymol import). `from rpg.story.model
# import MolAction` is ALLOWED here: MolAction is pure data (no pymol import;
# verified -- rpg/story/model.py imports only stdlib), AND molops.py is in
# the gate-excluded pymol_layer/ dir. The AST gate only bans pymol/PyQt5 in
# the DOMAIN tier (rpg/ excluding pymol_layer/ + ui/).
#
...
"""
from rpg.story.model import MolAction  # MolAction is pure data (no pymol) -- OK to import here
