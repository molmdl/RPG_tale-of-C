"""rpg/ui/achievements_dialog.py -- the achievements board dialog (ACH-01/ACH-02 UI).

A read-only ``QDialog`` that renders the collection-based achievement board
from an :class:`rpg.achievements.AchievementBoard` instance. The board (06-04)
...
module MAY import ``pymol.Qt`` (and the pure-Python ``rpg.achievements``
...
``@dataclass`` (3.7+). Matches the ``HelpDialog`` (``rpg/ui/help_dialog.py``)
conventions: ``QScrollArea`` wrapping a content widget + a Close button.
"""

from pymol.Qt import QtCore, QtGui, QtWidgets

from rpg.achievements import ACHIEVEMENT_CATALOG_V1
...
class AchievementsDialog(QtWidgets.QDialog):
    ...
    Args:
        achievement_board: the :class:`rpg.achievements.AchievementBoard`
