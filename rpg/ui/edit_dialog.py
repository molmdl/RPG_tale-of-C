"""rpg/ui/edit_dialog.py -- EditDialog (SC1 "edit interface"; EDIT-01/02/03 UI).

Gate-EXEMPT: ``rpg/ui/`` is in ``tools/check_imports.py`` SKIP_DIRS, so this
...
SCOPE). The dialog reads ``rpg/data/edits.json`` for the current enzyme's
...
whose :meth:`~rpg.story.model.EditIntent.signature` matches nothing and
always routes to the bad-ending pool (a valid but unfun outcome; the curated
list makes the edit a meaningful choice).

The edit routing is the EditRouter's job (``rpg/edit_router.py``), NOT the
dialog's. The dialog ONLY builds the ``(op, target, args)``; the controller
assembles the :class:`~rpg.story.model.EditIntent` via
:meth:`~rpg.ui.controller.Controller.build_edit_intent` and routes it via
:meth:`~rpg.ui.controller.Controller.apply_edit` (which calls
...
This plan (06-09) owns ONLY ``rpg/ui/edit_dialog.py``. It CONSUMES the seam
...
class EditDialog(QtWidgets.QDialog):
    ...
    from ``rpg/data/edits.json`` for the current ``enzyme_id`` + a few
    ...
    via :meth:`~rpg.ui.controller.Controller.build_edit_intent` and routes it
    via :meth:`~rpg.ui.controller.Controller.apply_edit`.
    ...
    Args:
        controller: the :class:`~rpg.ui.controller.Controller` instance. Held
    ...
    def selected_edit(self):
        ...
        EditIntent whose :meth:`~rpg.story.model.EditIntent.signature` is
    ...
    def _build_options(self, enzyme_id):
        ...
        """
        # Load edits.json cwd-independent via rpg.paths.data_path (the
