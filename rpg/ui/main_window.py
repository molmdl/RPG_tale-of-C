# rpg/ui/main_window.py -- Phase 6 Plan 06-08 the Qt MainWindow + StartDialog.
#
...
# domain-tier (rpg/ root) file imports pymol -- the boundary holds. Importing
...
"""rpg/ui/main_window.py -- Qt MainWindow + StartDialog (SC1 core UI surface).
...
from rpg.ui.widgets import StoryPanel, ChoicePanel
...
def _resolve_story_dir():
    ...
    Prefer the bundled (shipped) path ``rpg/data/story_glucose`` (where the
    ...
    build script only copies story_glucose into rpg/data/ at zip-build time, so
    in a bare repo checkout rpg/data/story_glucose/ does not exist). Returns
    the str path (or the shipped path if neither exists, letting
    ``StoryGraph.load`` raise a clear ``FileNotFoundError``).
    """
    shipped = rpg.paths.data_path("data", "story_glucose")
    if shipped.is_dir():
        return str(shipped)
    # Dev fallback: repo-root data/story_glucose. __file__ = rpg/ui/main_window.py
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        # EditRouter over the bundled rpg/data/edits.json lookup table.
    ...
    def render_turn(self, turn):
        ...
        # "stage:<x>" tags (rpg/story/model.py Node.tags); a node without a
    ...
    def _save(self):
        ...
        from rpg.ui.save_load_dialogs import ask_save_path  # lazy: 06-11 parallel
    ...
    def _load(self):
        ...
        from rpg.ui.save_load_dialogs import ask_load_path  # lazy: 06-11 parallel
    ...
    def _open_achievements(self):
        ...
        from rpg.ui.achievements_dialog import AchievementsDialog  # lazy
    ...
    def _open_help(self):
        ...
        from rpg.ui.help_dialog import HelpDialog  # lazy
    ...
    def _open_edit_dialog(self):
        ...
        at click time.
        """
        from rpg.ui.edit_dialog import EditDialog  # lazy: 06-09 parallel
