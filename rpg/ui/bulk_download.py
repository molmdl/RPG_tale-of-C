# rpg/ui/bulk_download.py -- Qt-free bulk-download runner (CAST-04).
#
...
# on disk; if any, it shows a modal dialog (rpg/ui/bulk_download_dialog.py)
...
# inject-cmd pattern):
#   * This module lives in rpg/ui/ (gate-EXEMPT via tools/check_imports.py
#     SKIP_DIRS) but DOES NOT import pymol.Qt -- it is the Qt-FREE runner.
#     It imports only `os`, `json`, `rpg.paths`, and
#     `rpg.pymol_layer.asset_manager.AssetManager` at module top. `cmd` is
...
#   * The QDialog wrapper (rpg/ui/bulk_download_dialog.py) IS gate-exempt
...
#
# GATE COMPLIANCE: although rpg/ui/ is in SKIP_DIRS, this module imports
# ONLY stdlib + rpg.* (no pymol.Qt) so it stays importable in pure WSL
...
as a function parameter, mirroring rpg.pymol_layer.asset_manager.AssetManager).
The QDialog wrapper (rpg/ui/bulk_download_dialog.py) calls these functions
...
Phase 6 caveat: the only download enzyme in rpg/data/cast.json is
...
def _default_cast_path(cast_path):
    ...
    The bundled default lives at rpg/data/cast.json (cwd-independent via
    rpg.paths.data_path -- __file__-relative, Pitfall 1 mitigation). Tests
    pass an explicit cast_path pointing at a temp file so they don't touch
    the bundled manifest.
    """
    if cast_path is not None:
        return cast_path
    return str(rpg.paths.data_path("data", "cast.json"))
...
def _downloaded_dir():
    ...
    Mirrors AssetManager._download_dir's path resolution (rpg.paths.data_path)
    ...
    runs.
    """
    return str(rpg.paths.data_path("data", "assets", "downloaded"))
