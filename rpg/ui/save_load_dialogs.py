"""rpg/ui/save_load_dialogs.py -- thin QFileDialog wrappers -> controller.save/load.
...
Matches the conventions of ``rpg/ui/help_dialog.py`` +
``rpg/ui/achievements_dialog.py`` (gate-EXEMPT, ``from pymol.Qt import ...``).
"""
import os
...
def _saves_dir():
    ...
    ``user_data_path`` resolves to ~/.pymol/rpg-tale-of-c/saves on Linux/Mac,
    %APPDATA%/pymol/rpg-tale-of-c/saves on Windows -- OUTSIDE the plugin
