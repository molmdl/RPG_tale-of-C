...
at ``user_data_path("achievements.json")`` (``~/.pymol/c14-tale-of-c/`` on
Linux/Mac, ``%APPDATA%/pymol/c14-tale-of-c/`` on Windows) -- OUTSIDE the
...
- Python 3.6 stdlib ONLY (``json``, ``os``, ``datetime``). NO pymol/PyQt5
  ...
  it lives in ``c14/`` root). Importable in pure WSL ``python3.6`` for unit
  tests.
- NO ``@dataclass`` (3.7+ -- research Pitfall 1). Plain class on instance
  attributes, matching the ``SaveStore`` (``c14/persist.py``) + ``Node``
  (``c14/story/model.py``) precedent.
