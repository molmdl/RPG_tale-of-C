...
from rpg.paths import data_path, selfcheck, user_data_path
...
class TestPathResolution(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._tmp = tempfile.mkdtemp(prefix="rpg_pathtest_")
        os.chdir(self._tmp)
    ...
    def test_data_path_returns_absolute_path(self):
        ...
        # repo_root = parent.parent, and rpg/ is under repo_root).
        pkg_root = Path(__file__).resolve().parent.parent / "rpg"
...
class TestUserDataPath(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._orig_cwd = os.getcwd()
        self._tmp = tempfile.mkdtemp(prefix="rpg_userdatatest_")
        os.chdir(self._tmp)
    ...
    def test_linux_mac_path_when_appdata_unset(self):
        # Linux/Mac branch: APPDATA unset -> ~/.pymol/rpg-tale-of-c/<parts>.
        os.environ.pop('APPDATA', None)
        p = user_data_path("achievements.json")
        expected_suffix = os.path.join(
            os.path.expanduser('~'), '.pymol', 'rpg-tale-of-c', 'achievements.json')
        self.assertTrue(
    ...
    def test_windows_path_when_appdata_set(self):
        # Windows branch: APPDATA set -> %APPDATA%/pymol/rpg-tale-of-c/<parts>.
        os.environ['APPDATA'] = 'C:\\Users\\test\\AppData\\Roaming'
        p = user_data_path("saves", "game.json")
        expected = os.path.join(
            'C:\\Users\\test\\AppData\\Roaming', 'pymol', 'rpg-tale-of-c',
            'saves', 'game.json')
        self.assertEqual(
            str(p), expected,
    ...
    def test_cwd_independence(self):
        ...
        tmp2 = tempfile.mkdtemp(prefix="rpg_userdatatest2_")
        try:
            os.chdir(tmp2)
    ...
    def test_multiple_parts(self):
        ...
        self.assertTrue(
            str(p).endswith(os.path.join('.pymol', 'rpg-tale-of-c', 'saves', 'sub', 'game.json')),
    ...
    def test_no_parts(self):
        ...
        os.environ.pop('APPDATA', None)
        p = user_data_path()
        expected = os.path.join(os.path.expanduser('~'), '.pymol', 'rpg-tale-of-c')
        self.assertEqual(
            str(p), expected,
            "no-parts base resolved {p}, expected {e}".format(p=p, e=expected))

    def test_gate_clean(self):
        # rpg.paths imports only os + pathlib (NO pymol/PyQt5) -> it imports
        ...
        import sys
        import rpg.paths  # noqa: F401  -- documents the gate-clean contract
        self.assertNotIn(
            'pymol', sys.modules,
            "rpg.paths must not import pymol (AST gate + runtime proof)")
        self.assertNotIn(
            'PyQt5', sys.modules,
            "rpg.paths must not import PyQt5 (AST gate + runtime proof)")
